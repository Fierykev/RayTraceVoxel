...
//--------------------------------------------------------------------------------------
int main()//int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
                                      ...
                                      void* pUserContext )
{
	...
	D3DXMatrixIdentity(&g_mPosition);

	D3DXMATRIXA16 mRotation;
	D3DXMatrixRotationY(&mRotation, -D3DX_PI/2.0f);
	g_mCenterMesh *= mRotation;

	D3DXMatrixTranslation(&mRotation, 0, 0, 400);
	g_mPosition *= mRotation;

	Create_Terrain_Shader();

	terrain.Load("Obj/Test.obj");

	// Shadow Map

	LoadShadowShader(pd3dDevice);

	shadowmap.CreateShadowShader(pd3dDevice);
                                  ...
                                  float fElapsedTime, void* pUserContext )
{
	...
	*/
	...
	mWorld = identity;//g_mCenterMesh *(*g_Camera.GetWorldMatrix()); correct identity for world later
	mProj = *g_Camera.GetProjMatrix();
	mView = *g_Camera.GetViewMatrix();
	...
	/*
	D3DXMATRIXA16 mRotation;
	D3DXMatrixRotationY(&mRotation, -D3DX_PI / 100.0f);
	g_mCenterMesh *= mRotation;
	*/
	//mWorld = g_mCenterMesh * g_mPosition;
	//mProj = *g_Camera.GetProjMatrix();
	...
	*/
	...
	//shadowmap.RenderShadowMapSetup(pd3dDevice, pd3dImmediateContext, mWorld, mProj, mView);

	//terrain.DrawNoTexture(pd3dImmediateContext);

	//EndShadowMapRender(pd3dDevice, pd3dImmediateContext);

	//shadowmap.SetupShadow(pd3dDevice, pd3dImmediateContext);

	// render mesh
	
	terrain.Draw(pd3dImmediateContext, vLightDir, mWorld, mProj, mView, g_Camera.GetEyePt());
